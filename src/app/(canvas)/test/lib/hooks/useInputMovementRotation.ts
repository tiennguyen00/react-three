...
import { inputControlsType } from "./useCharacterState"
...
function getModelRotation(inputs: inputControlsType) {
  ...
  if ((up || down) && !(down && up) && (left || right) && isMouseLooking) {
    ...
    const movementAngle = getSignedAngleBetweenVectors({ v1: movementDirection, v2: FORWARD })
...
export default function useInputMovementRotation(inputs: inputControlsType) {
