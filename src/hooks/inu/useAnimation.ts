...
const useAnimation = () => {
  ...
  // movement
  const changeCharacterState = (delta: number, character: Group, api: PublicApi) => {
    ...
    if (jump && Math.abs(newVelocity.y) < 0.001) {
      newVelocity.y += acc.y * delta
      api.velocity.set(newVelocity.x, 10, newVelocity.z)
    }
